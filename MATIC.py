...
def get_target_price(ticker, k):
    ...
    df = pyupbit.get_ohlcv(ticker, interval="hour", count=24)
...
def predict_price(ticker):
    ...
    closeDf = forecast[forecast['ds'] == forecast.iloc[-1]['ds'].replace(Minute=20)]
    if len(closeDf) == 0:
        closeDf = forecast[forecast['ds'] == data.iloc[-1]['ds'].replace(Minute=20)]
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            target_price = get_target_price("KRW-MATIC", 0.1)
