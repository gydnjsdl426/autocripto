...
def startGamble(name, num):
    try:
        ...
        elif upbit.get_avg_buy_price(name) * 0.97 > current_price or upbit.get_avg_buy_price(name) * 1.25 < current_price or (predicted_close_price[num] < current_price and upbit.get_avg_buy_price(name) * 1.025 < current_price) or current_price * 0.96 <= target_price or predict_price <= current_price * 0.95:
