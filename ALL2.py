    ...
    try:
        ...
        for n in name:
            if upbit.get_balance(n) == 0:
                if (current_price[n]*1.007 < predicted_min_price[i] or current_price[n]*1.020 < predicted_max_price[i]) and get_ma15(n) and krw > 5000:
                    upbit.buy_market_order(n, total*0.33)
        
            elif ((predicted_min_price[i] < current_price[n]*0.992 and predicted_close_price[i] < current_price[n] and predicted_max_price[i] < current_price[n]*1.01) 
